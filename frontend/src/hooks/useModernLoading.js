...
/**
 * Custom hook to ensure ModernLoadingAnimation displays for a minimum duration
 ...
 * @param {number} minDisplayTime - Minimum time in ms to show loader (default: 1000ms for testing)
 * @param {function} dataValidator - Optional function to validate if data is truly loaded
 * @returns {boolean} - Whether to show ModernLoadingAnimation
 * 
 * @example
 * const showModernLoading = useModernLoading(loading, accounts, MODERN_LOADING_MIN_DISPLAY_TIME);
 * if (showModernLoading) return <ModernLoadingAnimation />;
 */
const useModernLoading = (
  ...
  useEffect(() => {
    ...
    const shouldShowLoading = isLoading || !minTimePassed || !isDataLoaded;
