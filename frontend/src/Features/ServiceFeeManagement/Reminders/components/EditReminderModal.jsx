...
const EditReminderModal = ({ isOpen, onClose, onSave, reminderData }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    towerScope: 'All Towers', // Tower scope for hierarchy
    specificTarget: '',
  ...
  // Expand selected towers inline (show all) toggle
  const [expandTowers, setExpandTowers] = useState(false);
  // Expand selected units/residents inline toggles
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (showTowerDropdown || showUnitsDropdown || showResidentsDropdown) {
        ...
        const dropdowns = document.querySelectorAll('.border-gray-200.rounded-lg.bg-white.shadow-lg');
  ...
  useEffect(() => {
    ...
    loadData();
  }, [showTowerDropdown, showUnitsDropdown, showResidentsDropdown, towerResults.length, unitResults.length, residentResults.length]);
  ...
  const fetchTowers = async (searchTerm = '') => {
    try {
      ...
      // Normalize items so UI can always read `id` and `name`
      const normalized = results.map(item => ({
        ...item,
        id: item.id || item.tower__id || item.tower_id || null,
        name: item.name || item.tower__tower_name || item.tower_name || item.tower__name || item.tower__tower_number || item.tower_number || item.display_name || ''
      }));
      return normalized;
    } catch (error) {
      console.error('Error fetching towers:', error);
  ...
  const fetchUnits = async (searchTerm = '') => {
    try {
      ...
      const raw = Array.isArray(response.data) ? response.data : (response.data.results || response.data || []);
      const results = Array.isArray(raw) ? raw : [];
      // Normalize: ensure id and name fields exist for rendering
      const normalized = results.map(item => ({
        ...item,
  ...
  // Function to fetch display name for a specific ID (backward compatibility for single selection)
  const fetchDisplayNameForId = useCallback(async (id, audience) => {
    try {
      let results = [];
      let searchTerm = ''; // Empty search to get all results
      
      if (audience === 'Specific Tower') {
        results = await fetchTowers(searchTerm);
      } else if (audience === 'Specific Units') {
        results = await fetchUnits(searchTerm);
      } else if (audience === 'Specific Resident') {
        results = await fetchResidents(searchTerm);
      }
      
      // Find the item with matching ID
      const matchingItem = results.find(item => item.id === id);
      if (matchingItem) {
        setFormData(prev => ({ 
          ...prev, 
          specificTarget: matchingItem.name,
          specificTargetId: matchingItem.id
        }));
      }
    } catch (error) {
      console.error('Error fetching display name for ID:', error);
      // If we can't fetch the name, just show the ID
      setFormData(prev => ({ 
        ...prev, 
        specificTarget: id.toString()
      }));
    }
  }, []); // Empty dependency array since it only uses other functions and setState

  // Function to fetch multiple items by IDs
  const fetchItemsByIds = useCallback(async (idsString, audience) => {
    try {
      ...
      console.log('Fetching items for IDs:', ids, 'Audience:', audience);

      // Helper to extract possible id values from API objects
      ...
      // Helper to normalize an item into {id, name, type, ...rest}
      const normalize = (item, defaultType) => {
        const id = extractIds(item)[0] || item.id || null;
        const name = item.name || item.unit_name || item.unit__name || item.tower__tower_name || item.tower_name || item.full_name || item.display_name || item.resident_name || '';
        return { ...item, id: Number(id), name, type: defaultType || item.type };
      };
      ...
      if (audience === 'Specific Tower') {
        ...
        allResults = towers.map(t => normalize(t, 'tower'));
      } else if (audience === 'Specific Units') {
        const [towers, units] = await Promise.all([
          fetchTowers(''),
          fetchUnits('')
        ...
        allResults = [
          ...units.map(u => normalize(u, 'unit')),
          ...towers.map(t => normalize(t, 'tower'))
        ];
      } else if (audience === 'Specific Resident') {
        const residents = await fetchResidents('');
        allResults = residents.map(r => normalize(r, 'resident'));
      }

      // Match ids against any recognized id field
      const matchedItems = allResults.filter(item => {
        const itemIds = extractIds(item);
        return itemIds.some(i => ids.includes(i)) || ids.includes(Number(item.id));
      }).map(item => ({ ...item }));
  ...
  const handleTimeChange = (timeIndex, newTime) => {
    ...
    const loadReminderData = async () => {
      if (reminderData && isOpen) {
        setIsLoadingInitialData(true);
        console.log('=== LOADING REMINDER DATA ===');
        console.log('Full reminderData:', reminderData);
        console.log('Available fields:', Object.keys(reminderData));
        
        try {
          // PRIORITY 1: Use new normalized send_when_data from API
          let sendWhenArray = [];
          let times = [];  // Initialize times array
          
          if (reminderData.send_when_data && Array.isArray(reminderData.send_when_data) && reminderData.send_when_data.length > 0) {
            console.log('Loading from new send_when_data:', reminderData.send_when_data);
            sendWhenArray = reminderData.send_when_data.map((timing) => ({
              type: timing.timing_type,  // 'before_due', 'after_due', 'on_due', 'specific'
              day: timing.day_offset,     // 0, 1, 3, 5, 7, etc.
              label: timing.timing_label  // "3 days before due", "on due date", etc.
            }));
            console.log('Converted send_when_data to sendWhenArray:', sendWhenArray);

            // Use master send_times from reminder
            const masterTimes = reminderData.send_times && Array.isArray(reminderData.send_times)
              ? reminderData.send_times.filter(Boolean)
              : [];
            times = masterTimes;
            console.log('Loaded master send_times:', times);
            
            // Add any custom options to quickOptions if they don't already exist
            sendWhenArray.forEach(option => {
              const exists = quickOptions.some(opt => opt.type === option.type && opt.day === option.day);
              if (!exists) {
                setQuickOptions(prev => [...prev, option]);
                console.log('Added custom quick option from send_when_data:', option);
              }
            });
          } else {
            // PRIORITY 2: Fall back to old send_when_type/send_when_day structure
            ...
            console.log('Loading from old format - sendWhenTypes:', sendWhenTypes, 'sendWhenDays:', sendWhenDays);
            
            // Handle arrays (new format)
            if (Array.isArray(sendWhenTypes) && Array.isArray(sendWhenDays) && sendWhenTypes.length === sendWhenDays.length) {
              sendWhenArray = sendWhenTypes.map((type, index) => {
                ...
                return { type: type, day: day, label: label };
              });
              
              // Add any custom quick options (that don't already exist) to the quickOptions list
              sendWhenArray.forEach(option => {
                const exists = quickOptions.some(opt => opt.type === option.type && opt.day === option.day);
                if (!exists) {
                  setQuickOptions(prev => [...prev, option]);
                  console.log('Added custom quick option:', option);
                }
              });
              
              console.log('Created sendWhen array from arrays:', sendWhenArray);
            }
            // Handle single values (backward compatibility)
              ...
              sendWhenArray = [{ type: sendWhenTypes, day: day, label: label }];
              console.log('Created sendWhen object from single values:', sendWhenArray);
            }
          }
          
          // Load times from master send_times if not already set
          if (times.length === 0) {
            if (reminderData.send_times && Array.isArray(reminderData.send_times)) {
              times = reminderData.send_times.filter(Boolean);
              console.log('Loaded times from reminder.send_times:', times);
            }
          }
          
          // PRIORITY 1: Use new normalized payment_status_data from API
          let paymentStatus = [];
          if (reminderData.payment_status_data && Array.isArray(reminderData.payment_status_data)) {
            paymentStatus = reminderData.payment_status_data.map(ps => ps.status);
            console.log('Loaded paymentStatus from payment_status_data (raw):', paymentStatus);
            // Normalize to display-case values used in the UI and avoid legacy 'All' token
            paymentStatus = Array.from(new Set(paymentStatus.map(s => {
              const v = (s || '').toString().toLowerCase();
              if (v === 'paid') return 'Paid';
              if (v === 'due') return 'Due';
              if (v === 'overdue') return 'Overdue';
              // treat 'all' as full set (handled below)
              return s;
            })));
            // If legacy 'All' was present, replace with explicit statuses
            if (paymentStatus.includes('All') || paymentStatus.map(p => (p || '').toString().toLowerCase()).includes('all')) {
              paymentStatus = ['Paid', 'Due', 'Overdue'];
            }
            console.log('Normalized paymentStatus for UI:', paymentStatus);
          } else {
            paymentStatus = reminderData.paymentStatus || reminderData.payment_status || ['Paid', 'Due', 'Overdue'];
            console.log('Loaded paymentStatus from old format:', paymentStatus);
          }
          
          // Handle channels data - backend sends individual fields, frontend expects nested object
          const channels = {
            appNotification: reminderData.appNotification || reminderData.app_notification || false,
            sms: reminderData.sms || false,
            email: reminderData.email || false
          };
          console.log('Processed channels:', channels);
          
          // PRIORITY 1: Use new normalized tower_data from API
          let selectedTowers = [];
          if (reminderData.tower_data && Array.isArray(reminderData.tower_data)) {
            selectedTowers = reminderData.tower_data.map(t => ({
              id: t.tower__id,
              name: t.tower__tower_name,
              number: t.tower__tower_number
            }));
            console.log('Loaded towers from tower_data:', selectedTowers);
          } else {
            // Fall back to old format
            selectedTowers = reminderData.selectedTowers || [];
            const towerIds = reminderData.towerIds || reminderData.tower_id;
            console.log('Tower IDs from reminder:', towerIds, 'Type:', typeof towerIds);
            
            if (Array.isArray(towerIds) && towerIds.length > 0 && selectedTowers.length === 0) {
              console.log('🔍 Fetching towers from tower_id field...');
              try {
                selectedTowers = await fetchItemsByIds(towerIds.join(','), 'Specific Tower');
                console.log('✅ Loaded towers from tower_id:', selectedTowers);
              } catch (error) {
                console.error('❌ Error fetching towers:', error);
              }
            }
          ...
          let selectedUnits = reminderData.selectedUnits || [];
          let selectedResidents = reminderData.selectedResidents || [];
          
          // PRIORITY 1: Use new normalized specific_target_data from API
          if (reminderData.specific_target_data && Array.isArray(reminderData.specific_target_data) && reminderData.specific_target_data.length > 0) {
            console.log('Loading specific targets from specific_target_data:', reminderData.specific_target_data);
            
            // Group by target_type and collect IDs
            const byType = reminderData.specific_target_data.reduce((acc, target) => {
              if (!acc[target.target_type]) acc[target.target_type] = [];
              acc[target.target_type].push(target.target_id);
              return acc;
            }, {});
            
            // Fetch units if present
            if (byType.unit && byType.unit.length > 0) {
              console.log('Found units in specific targets, fetching:', byType.unit);
              try {
                selectedUnits = await fetchItemsByIds(byType.unit.join(','), 'Specific Units');
                console.log('✅ Loaded units from specific_target_data:', selectedUnits);
              } catch (error) {
                console.error('❌ Error fetching units from specific_target_data:', error);
              }
            }
            
            // Fetch residents if present
            if (byType.resident && byType.resident.length > 0) {
              console.log('Found residents in specific targets, fetching:', byType.resident);
              try {
                selectedResidents = await fetchItemsByIds(byType.resident.join(','), 'Specific Resident');
                console.log('✅ Loaded residents from specific_target_data:', selectedResidents);
              } catch (error) {
                console.error('❌ Error fetching residents from specific_target_data:', error);
              }
            }
          } else {
            // Fall back to old format
            const specificTarget = reminderData.specificTarget || reminderData.specific_target || '';
            console.log('Specific target value:', specificTarget, 'Type:', typeof specificTarget);
            
            // Check if specificTarget is a JSON array
            let hasTargetIds = false;
            if (Array.isArray(specificTarget)) {
              hasTargetIds = specificTarget.length > 0;
            } else if (typeof specificTarget === 'string' && specificTarget.trim().length > 0) {
              hasTargetIds = true;
            }
            
            console.log('Has target IDs:', hasTargetIds, 'Audience:', reminderData.audience);
            
            // Fetch items by IDs if we have target IDs
            if (hasTargetIds && reminderData.audience && 
                selectedTowers.length === 0 && selectedUnits.length === 0 && selectedResidents.length === 0) {
              console.log('🔍 Attempting to fetch items for IDs from specific_target:', specificTarget);
              
              // Convert to string format for fetchItemsByIds
              const targetString = Array.isArray(specificTarget) ? specificTarget.join(',') : String(specificTarget);
              
              if (reminderData.audience === 'Specific Tower') {
                console.log('Fetching towers...');
                selectedTowers = await fetchItemsByIds(targetString, reminderData.audience);
                console.log('✅ Fetched towers:', selectedTowers);
              } else if (reminderData.audience === 'Specific Units') {
                console.log('Fetching units...');
                selectedUnits = await fetchItemsByIds(targetString, reminderData.audience);
                console.log('✅ Fetched units:', selectedUnits);
              } else if (reminderData.audience === 'Specific Resident') {
                console.log('Fetching residents...');
                selectedResidents = await fetchItemsByIds(targetString, reminderData.audience);
                console.log('✅ Fetched residents:', selectedResidents);
              }
            } else {
              console.log('❌ Skipping fetch - hasTargetIds:', hasTargetIds, 'selectedTowers.length:', selectedTowers.length, 'selectedUnits.length:', selectedUnits.length, 'selectedResidents.length:', selectedResidents.length);
            }
          }
          
          console.log('📝 Setting form data with:');
          console.log('- audience (raw):', reminderData.audience);
          console.log('- sendWhen:', sendWhenArray);
          console.log('- times:', times);
          console.log('- paymentStatus:', paymentStatus);
          console.log('- selectedTowers:', selectedTowers);
          console.log('- selectedUnits:', selectedUnits);
          console.log('- selectedResidents:', selectedResidents);
          
          // Infer audience if backend didn't provide it explicitly
          // Prefer the more specific audience: Units > Residents. Do not set Towers as an audience option.
          let inferredAudience = reminderData.audience;
          if (!inferredAudience) {
            if (selectedUnits && selectedUnits.length > 0) {
              inferredAudience = 'Specific Units';
            } else if (selectedResidents && selectedResidents.length > 0) {
              inferredAudience = 'Specific Resident';
            } else {
              inferredAudience = 'All Towers';
            }
            console.log('Inferred audience:', inferredAudience);
          } else if (inferredAudience === 'Specific Tower') {
            // Map legacy 'Specific Tower' audience to 'All Towers' by default (tower-specific audience option removed)
            inferredAudience = (selectedUnits && selectedUnits.length > 0) ? 'Specific Units' : (selectedResidents && selectedResidents.length > 0) ? 'Specific Resident' : 'All Towers';
            console.log('Mapped legacy Specific Tower audience to:', inferredAudience);
          }

          // If units were loaded, prefer Units as the audience so UI reflects selections
          const finalAudience = (selectedUnits && selectedUnits.length > 0) ? 'Specific Units' : inferredAudience;

          setFormData({
            reminderName: reminderData.reminderName || reminderData.reminder_name || '',
            reminderType: reminderData.reminderType || reminderData.reminder_type || 'Scheduled',
            sendWhen: sendWhenArray,
            times: times,
            channels: channels,
            audience: finalAudience,
            paymentStatus: paymentStatus,
            specificTarget: '', // Clear this as we're using selected arrays
            specificTargetId: null,
            selectedTowers: selectedTowers,
            selectedUnits: selectedUnits,
            selectedResidents: selectedResidents,
            searchResults: [],
            messagePreview: reminderData.messagePreview || reminderData.message_preview || ''
          });
          
          console.log('✅ Form data updated successfully');
          console.log('=== END LOADING REMINDER DATA ===');
        } catch (error) {
    ...
    loadReminderData();
  }, [reminderData, isOpen, fetchItemsByIds]);

  // Clear error message when modal opens or closes
  useEffect(() => {
    if (isOpen) {
      setError('');
      setFieldErrors({});
    } else {
      // Clear error and reset loading when modal closes
      setError('');
      setFieldErrors({});
      setLoading(false);
      setShowTowerDropdown(false);
  ...
  // Handle selection from dropdown (for single selection - backward compatibility)
  ...
  const handleTowerToggle = (tower) => {
    ...
    // Clear dependent caches so they will re-fetch with tower filters as needed
    setUnitResults([]);
    setResidentResults([]);
  };
  ...
  const handleAddQuickOption = () => {
    ...
    if (!exists) {
      ...
      // Automatically check the new option
      setFormData(prev => ({
        ...prev,
        sendWhen: [...prev.sendWhen, newOption]
      }));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        // Send appropriate ID fields based on selections
        // Always include unitIds when units are selected, regardless of audience
      ...
      console.log('Submitting reminder update payload:', submitData);
      ...
      let errorMessage = 'Unable to update the reminder. Please try again.';
  ...
  const handleSendNow = async (e) => {
    ...
    try {
      ...
      console.log('Sending reminder now payload:', sendData);
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50">
        <div className="flex items-center justify-center min-h-screen p-4">
          ...
          {/* Modal Body */}
          <form id="editReminderForm" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`login-field-input text-sm bg-white ${
                      fieldErrors.reminderName ? 'border-red-500' : ''
                    }`}
                    disabled={loading}
                  />
                ...
                {/* Audience - Hidden, tower-based hierarchy instead */}
                {/* <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <div className="mb-6 relative">
                  ...
                  <div className="mb-3">
                    <button
                      ...
                      className={`login-field-input !flex justify-between items-center gap-2 whitespace-nowrap ${showTowerDropdown ? '!border-primary !bg-white !shadow-ring-primary cursor-pointer' : 'bg-white cursor-pointer hover:border-primary hover:bg-gray-50'} ${isLoadingInitialData ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                          ...
                          return (
                            <>
                              ...
                              {shouldShowToggle && (
                                <button
                                  type="button"
                                  onClick={(e) => { e.stopPropagation(); setExpandTowers(prev => !prev); }}
                  ...
                  {/* Tower Dropdown */}
                  {showTowerDropdown && (
                    <div className="absolute z-50 mt-1 w-full left-0 top-full border border-gray-200 rounded-lg bg-white shadow-lg" onClick={(e) => e.stopPropagation()}>
                      {/* Select All Option */}
                      ...
                      <div className="p-3 border-t border-gray-200 flex justify-between space-x-2">
                        <button
                          type="button"
                          onClick={() => setFormData(prev => ({ ...prev, selectedTowers: [] }))}
                        ...
                        <button
                          type="button"
                          onClick={() => setShowTowerDropdown(false)}
                          className="px-3 py-1 bg-primary text-white text-sm rounded hover:bg-primaryHover transition-colors"
                        >
                          OK
                        </button>
                ...
                {/* Audience selection intentionally hidden in Edit modal.
                    Audience will be inferred automatically from loaded targets
                    (Units > Residents), and unit selections will always be
                    sent in the payload when present.
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Audience <span className="text-primary text-lg">*</span>
                  </label>
                ...
                */}
                ...
                {/* Multiple Units Selection (Edit modal parity) */}
                {formData.audience === 'Specific Units' && (
                  <div>
                    ...
                    <div className="mb-3 relative">
                      ...
                      {/* Units Dropdown (re-use searchResults) positioned absolutely */}
                      {showUnitsDropdown && (
                    ...
                    {/* Selected Units Display (show up to 6, +N more) */}
                    {formData.selectedUnits.length > 0 && (
                      <div className="mb-3">
                    ...
                    {/* Units Dropdown (re-use searchResults) */}
                    {showUnitsDropdown && (
                ...
                {/* Multiple Residents Selection (Edit modal parity) */}
                {formData.audience === 'Specific Resident' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    {/* Selected Residents Display (show up to 6, +N more) */}
                    {formData.selectedResidents.length > 0 && (
                      <div className="mb-3">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows={15}
                    className={`login-field-input text-sm resize-none ${
                      fieldErrors.messagePreview ? 'border-red-500' : ''
                    }`}
          ...
          <div className="p-6 border-t border-gray-200 bg-white flex justify-end space-x-4">
            ...
            <button
              ...
              disabled={loading || !(Array.isArray(formData.paymentStatus) && formData.paymentStatus.length > 0)}
              className={`px-6 py-2 text-white rounded transition-colors flex items-center gap-2 ${
                (loading || !(Array.isArray(formData.paymentStatus) && formData.paymentStatus.length > 0))
