...
const EditCategoryModal = ({ isOpen, onClose, onSubmit, category }) => {
  ...
  // Check if any fields have changed from original
  const hasChanges = () => {
    ...
    return (
      ...
      formData.icon !== originalDataRef.current.icon ||
      formData.color !== originalDataRef.current.color
    );
  };
      ...
      <div className="bg-white rounded-xl md:rounded-2xl shadow-xl max-w-lg w-full max-h-[95vh] md:max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-start sm:items-center p-4 md:p-6 border-b border-gray-200 gap-3">
          <div className="flex-1 min-w-0">
            <h2 className="text-xl md:text-2xl font-bold text-gray-900">Edit Category</h2>
            <p className="text-xs md:text-sm text-gray-600 mt-1">
              Update bill category information
            </p>
          </div>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-4 md:space-y-6">
                ...
                return (
                  <button
                    ...
                    className={`w-14 h-10 md:w-20 md:h-10 rounded-lg ${color.bg} transition-all flex-shrink-0 ${
                      isSelected
                        ? 'ring-2 md:ring-4 ring-offset-1 md:ring-offset-2 ring-gray-400'
                        : 'hover:scale-110'
                    }`}
                    title={color.name}
                  />
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            <button
              type="submit"
              disabled={!hasChanges()}
              className={`flex-1 px-4 md:px-6 py-2.5 md:py-3 rounded-lg transition-colors font-medium text-sm md:text-base w-full sm:w-auto ${
                hasChanges()
