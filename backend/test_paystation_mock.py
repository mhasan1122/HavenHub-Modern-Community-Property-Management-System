...
if payment:
    print(f"Testing with payment ID: {payment.id}, Unit: {payment.unit.unit_name if payment.unit else 'None'}")
    try:
        ...
        for notif in notifs:
            print(f" - Notif {notif.id}: recipient={notif.recipient.id} ({notif.recipient.full_name}), title='{notif.title}', channel='{notif.channel}'")
