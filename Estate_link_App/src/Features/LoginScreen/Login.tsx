...
import { loginUser, checkUserStatus, clearError, setError } from 'store/slices/authSlice';
...
export function Login() {
  ...
  const handleInputChange = async (field: string, value: string) => {
    ...
    if (field === 'username') {
      ...
      if (value.trim()) {
      try {
        await loginSchema.validateAt('username', {
          username: value,
  ...
  const handleInputFocus = () => {
    setKeyboardVisible(true);
    setShowSuggestions(true);
    // refresh full list if field empty
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Don't navigate to other screens on error - let Redux handle the error display
      // The error will be shown in the UI through the Redux error state
    }
  };
            ...
            <View 
              ...
              <View className="items-center space-y-6">
                ...
                <TouchableOpacity 
                  ...
                  disabled={isLoading}>
                  <Text
                    className={`font-oxanium-bold text-text-primary underline font-normal ${isSmallScreen ? 'text-base' : 'text-lg'}`}>
